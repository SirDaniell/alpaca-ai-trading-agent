...
class TechnicalIndicators:
    ...
    def _calculate_ma_differences(self, df: pd.DataFrame, diff_col: str, reporter: Optional[ProgressReporter] = None):
        ...
        new_cols["Volume_Change_Pct"] = df["Volume"].pct_change().replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0)
        
        if "Bar_Volume_Up" in df.columns:
            new_cols["Up_Volume_Change_Pct"] = df["Bar_Volume_Up"].pct_change().replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0)
        if "Bar_Volume_Down" in df.columns:
            new_cols["Down_Volume_Change_Pct"] = df["Bar_Volume_Down"].pct_change().replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0)
        ...
        for period in self.config.rsi_periods:
            try:
                ...
                if period == self.config.rsi_periods[0]:
                    ...
                    new_cols[f"RSI_{period}_Pct_Change"] = pct_change.replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0)
    ...
    def _calculate_volume_indicators(self, df: pd.DataFrame, calculate_last_n: Optional[int] = None, reporter: Optional[ProgressReporter] = None) -> pd.DataFrame:
        ...
        for source_col, target_col in volume_change_cols:
            ...
            df[target_col] = df[target_col].replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0)
    ...
    def _calculate_smc_indicators(self, df: pd.DataFrame, reporter: Optional[ProgressReporter] = None) -> pd.DataFrame:
        ...
        # FVG centre diff
        df["FVG_Diff"] = df["SMC_FVG_FVG"].ffill() - df[diff_col]
        # FVG top / bottom distance to price
        df["SMC_FVG_Top_Diff"]    = df["SMC_FVG_Top"].ffill()    - df[diff_col]
        df["SMC_FVG_Bottom_Diff"] = df["SMC_FVG_Bottom"].ffill() - df[diff_col]
        # Order Block top / bottom distance to price
        df["SMC_OB_Top_Diff"]    = df["SMC_OB_Top"].ffill()    - df[diff_col]
        df["SMC_OB_Bottom_Diff"] = df["SMC_OB_Bottom"].ffill() - df[diff_col]
        # Swing level distance to price
        df["SMC_Swing_Level_Diff"] = df["SMC_Swing_Level"].ffill() - df[diff_col]
        
        # Liquidity level diff - handle case where no liquidity zones exist
        # If SMC_Liquidity_Level is all NaN (no zones detected), the diff should be sentinel
        # This will be handled in _finalize_dataframe where we fill liquidity columns with sentinel
        df["SMC_Liquidity_Level_Diff"] = df["SMC_Liquidity_Level"].ffill() - df[diff_col]
    ...
    def _calculate_snr_vix_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        new_cols["SNR"] = snr.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    def _calculate_candle_structure(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        log_c  = np.log(safe_c)
        ret_5  = (log_c - log_c.shift(5)).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        ret_20 = (log_c - log_c.shift(20)).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    def _calculate_candle_bull_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ret_1    = ret_1.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    def _calculate_price_velocity(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "ATR" in df.columns:
            ...
            bull_vel_norm = (bull_vel_raw / atr_per_s).replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0.0)
            bear_vel_norm = (bear_vel_raw / atr_per_s).replace([np.inf, -np.inf], np.nan).infer_objects(copy=False).fillna(0.0)
        else:
            ...
            bull_vel_norm = (bull_vel_raw / bull_max).replace([np.inf, -np.inf], np.nan).fillna(0.0)
            bear_vel_norm = (bear_vel_raw / bear_max).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        ...
        new_cols["Price_Velocity_Net"] = net_vel.replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-1.0, 1.0)
    ...
    def _calculate_volatility_regime(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        expansion = (short_std / long_std).replace([np.inf, -np.inf], np.nan)
        ...
        new_cols["Volatility_Bull"] = (bull_std / total_std).replace([np.inf, -np.inf], np.nan).clip(0.0, 1.0).fillna(0.5)
        new_cols["Volatility_Bear"] = (bear_std / total_std).replace([np.inf, -np.inf], np.nan).clip(0.0, 1.0).fillna(0.5)
    ...
    def _calculate_regime_speed(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        new_cols["Regime_Speed_Divergence"] = divergence.replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-1.0, 1.0)
