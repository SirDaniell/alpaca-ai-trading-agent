...
class TechnicalIndicators:
    ...
    def _deduplicate_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """Ensure df has unique column names (keep first occurrence)."""
        if df is not None and hasattr(df, 'columns') and df.columns.duplicated().any():
            dup_cols = df.columns[df.columns.duplicated()].unique().tolist()
            logger.warning("⚠️ [TI] Deduplicating %d duplicate column(s): %s", len(dup_cols), dup_cols[:10])
            return df.loc[:, ~df.columns.duplicated(keep="first")].copy()
        return df

    def _get_series(self, df: pd.DataFrame, col: str, default: float = np.nan) -> pd.Series:
        """Safely retrieve a 1D Series for col from df, handling duplicates and missing columns."""
    ...
    def _clean_inf(self, s: Union[pd.Series, pd.DataFrame], fill_value: Optional[float] = 0.0) -> pd.Series:
        """Safely replace inf/-inf in a Series or DataFrame column without triggering Pandas downcasting or type errors."""
        if s is None:
            return s
    ...
    def _safe_concat(self, df: pd.DataFrame, new_data: Union[pd.DataFrame, Dict]) -> pd.DataFrame:
        """
        Safely append new columns/DataFrame to `df` without introducing duplicate column names.
        If any column in `new_data` already exists in `df`, the existing column is replaced.
        """
        ...
        if isinstance(new_data, dict):
            ...
            for k, v in new_data.items():
                if isinstance(v, pd.DataFrame):
                    sanitized_dict[k] = v.iloc[:, 0]
                elif isinstance(v, pd.Series):
                    sanitized_dict[k] = v
                else:
                    sanitized_dict[k] = pd.Series(v, index=df.index)
            new_df = pd.DataFrame(sanitized_dict, index=df.index)
        else:
            if new_data is None or new_data.empty:
                return df
            new_df = new_data
            
        new_df = self._deduplicate_df(new_df)

        cols_to_drop = [col for col in new_df.columns if col in df.columns]
        if cols_to_drop:
    ...
    def _prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 0.5 Post-rename column deduplication
        if df.columns.duplicated().any():
            df = df.loc[:, ~df.columns.duplicated(keep='first')].copy()
    ...
    def _calculate_basic_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if new_cols:
            df = self._safe_concat(df, new_cols)
    ...
    def _calculate_ma_differences(self, df: pd.DataFrame, diff_col: str, reporter: Optional[ProgressReporter] = None):
        ...
        long_ma_25  = self._get_series(df, "Long_MA_25")
        short_ma_10 = self._get_series(df, "Short_MA_10")
        long_ma_100 = self._get_series(df, "Long_MA_100")
        short_ma_50 = self._get_series(df, "Short_MA_50")
        price_s     = self._get_series(df, diff_col)

        new_cols["Short_Period_MA_Diff"] = long_ma_25 - short_ma_10
        new_cols["Long_Period_MA_Diff"] = long_ma_100 - short_ma_50
        new_cols["Price_Short_Period_Diff"] = price_s - short_ma_10
        new_cols["Price_Short_Long_Period_Diff"] = price_s - long_ma_25
        new_cols["Price_Long_Short_Period_Diff"] = price_s - short_ma_50
        new_cols["Price_Long_Long_Period_Diff"] = price_s - long_ma_100

        vol_s = self._get_series(df, "Volume", 0.0)
        new_cols["Volume_Change_Pct"] = self._clean_inf(vol_s.pct_change(), 0)
        
        if "Bar_Volume_Up" in df.columns:
            new_cols["Up_Volume_Change_Pct"] = self._clean_inf(self._get_series(df, "Bar_Volume_Up").pct_change(), 0)
        if "Bar_Volume_Down" in df.columns:
            new_cols["Down_Volume_Change_Pct"] = self._clean_inf(self._get_series(df, "Bar_Volume_Down").pct_change(), 0)
        # Price vs config-driven Short_MA / Long_MA
        if "Short_MA" in df.columns:
            new_cols["Short_MA_Diff"] = price_s - self._get_series(df, "Short_MA")
        if "Long_MA" in df.columns:
            new_cols["Long_MA_Diff"] = price_s - self._get_series(df, "Long_MA")
        ...
        for ma_col, diff_name in named_ma_diffs:
            if ma_col in df.columns:
                new_cols[diff_name] = self._get_series(df, ma_col) - price_s

        # ── EMA diffs — driven by config.ema_periods (no hardcoded list) ────
        for period in self.config.ema_periods:
            ema_col = f"EMA_{period}"
            if ema_col in df.columns:
                new_cols[f"EMA_{period}_Diff"] = self._get_series(df, ema_col) - price_s
        ...
        for period in self.config.ema_periods:
            if period != base_ema_period:
                ...
                if ema_col in df.columns and base_ema_col in df.columns:
                    new_cols[f"EMA_{period}_Minus_EMA{base_ema_period}"] = self._get_series(df, ema_col) - self._get_series(df, base_ema_col)

        # ── MA-200 vs lagged price (trend context) ───────────────────────────
        if "MA_200" in df.columns:
            ma_200 = self._get_series(df, "MA_200")
            for i in range(5):
                new_cols[f"MA_200_Change_{i}"] = ma_200 - price_s.shift(i)

        # ── Dynamic SMA diffs (from config.sma_periods) ─────────────────────
        for length in self.config.sma_periods:
            sma_col = f"SMA_{length}"
            if sma_col in df.columns:
                new_cols[f"SMA_{length}_Diff"] = self._get_series(df, sma_col) - price_s

        # ── SMA_100 vs SMA_50 spread ─────────────────────────────────────────
        if "SMA_100" in df.columns and "SMA_50" in df.columns:
            new_cols["MA_100_50_Diff"] = self._get_series(df, "SMA_100") - self._get_series(df, "SMA_50")
        else:
    ...
    def _calculate_lettered_variables(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = self._deduplicate_df(df)
        diff_col = self.config.diff_column
        diff_series = self._get_series(df, diff_col)
        ...
        for ma_col, letter in ma_mappings:
            s = self._get_series(df, ma_col)
            if s is not None:
                new_cols[letter] = s - diff_series
        ...
        for ema_col, letter in ema_mappings:
            s = self._get_series(df, ema_col)
            if s is not None:
                new_cols[letter] = s - diff_series
        
        return self._safe_concat(df, new_cols)
    ...
    def _calculate_ema_change_series(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        df = self._deduplicate_df(df)
        base_s = self._get_series(df, "EMA_8")
        if base_s is None:
            return df
        ...
        for idx, period in enumerate(change_periods):
            s = self._get_series(df, f"EMA_{period}")
            if s is not None:
                new_cols[f"MA_Change{idx}"] = s - base_s
        ...
        for period in self.config.rsi_periods:
            try:
                ...
                if period == self.config.rsi_periods[0]:
                    ...
                    new_cols[f"RSI_{period}_Pct_Change"] = self._clean_inf(pct_change, 0)
    ...
    def _calculate_volume_indicators(self, df: pd.DataFrame, calculate_last_n: Optional[int] = None, reporter: Optional[ProgressReporter] = None) -> pd.DataFrame:
        ...
        missing_swings = [c for c in swing_cols if c not in df.columns]
        if missing_swings:
            df = self._safe_concat(df, pd.DataFrame(np.nan, index=df.index, columns=missing_swings))
        ...
        for source_col, target_col in volume_change_cols:
            ...
            new_volume_cols[target_col] = self._clean_inf(pct_change, 0)
            
        if new_volume_cols:
            df = self._safe_concat(df, new_volume_cols)
    ...
    def _calculate_trendlines(self, df: pd.DataFrame, calculate_last_n: Optional[int] = None, reporter: Optional[ProgressReporter] = None) -> pd.DataFrame:
        ...
        missing_trendline = [c for c in trendline_cols if c not in df.columns]
        if missing_trendline:
            df = self._safe_concat(df, pd.DataFrame(np.nan, index=df.index, columns=missing_trendline))
        ...
        diff_s = self._get_series(df, diff_col)
        df["Support_Trendline_Diff"] = self._get_series(df, "Support_Trendline_Value") - diff_s
        df["Resist_Trendline_Diff"] = self._get_series(df, "Resist_Trendline_Value") - diff_s
    ...
    def _calculate_heikinashi(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        missing_ha = [c for c in ha_cols if c not in df.columns]
        if missing_ha:
            df = self._safe_concat(df, pd.DataFrame(np.nan, index=df.index, columns=missing_ha))
    ...
    def _calculate_doji(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Doji" not in df.columns:
            df = self._safe_concat(df, pd.DataFrame({"Doji": np.nan}, index=df.index))
        else:
            df["Doji"] = np.nan
    ...
    def _calculate_smc_indicators(self, df: pd.DataFrame, reporter: Optional[ProgressReporter] = None) -> pd.DataFrame:
        ...
        if missing_smc:
            df = self._safe_concat(df, pd.DataFrame(np.nan, index=df.index, columns=missing_smc))
        ...
        smc_new_cols = {}
        diff_s = self._get_series(df, diff_col)
        smc_new_cols["FVG_Diff"] = self._get_series(df, "SMC_FVG_FVG").ffill() - diff_s
        smc_new_cols["SMC_FVG_Top_Diff"]    = self._get_series(df, "SMC_FVG_Top").ffill()    - diff_s
        smc_new_cols["SMC_FVG_Bottom_Diff"] = self._get_series(df, "SMC_FVG_Bottom").ffill() - diff_s
        smc_new_cols["SMC_OB_Top_Diff"]    = self._get_series(df, "SMC_OB_Top").ffill()    - diff_s
        smc_new_cols["SMC_OB_Bottom_Diff"] = self._get_series(df, "SMC_OB_Bottom").ffill() - diff_s
        smc_new_cols["SMC_Swing_Level_Diff"] = self._get_series(df, "SMC_Swing_Level").ffill() - diff_s
        smc_new_cols["SMC_Liquidity_Level_Diff"] = self._get_series(df, "SMC_Liquidity_Level").ffill() - diff_s

        df = self._safe_concat(df, smc_new_cols)
    ...
    def _add_legacy_aliases(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if new_alias_cols:
            df = self._safe_concat(df, new_alias_cols)
    ...
    def _calculate_snr_vix_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        new_cols["VIX_20"] = self._clean_inf(vix_term.rolling(window=window).mean() ** 0.5 * 100.0, 0.0)
    ...
    def _calculate_candle_bull_score(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Candle Bull Score [0, 1] — unified metric of candle bullishness.
        ...
        if "RSI" in df.columns:
            ...
            for col in df.columns:
                if col.startswith("RSI_") and col.replace("RSI_", "").isdigit():
                    rsi_col = col
                    break

        if rsi_col:
            rsi_norm = (self._get_series(df, rsi_col) / 100.0).clip(0.0, 1.0)
        else:
            rsi_norm = pd.Series(0.5, index=df.index)

        diff_col = self.config.diff_column
        close    = self._get_series(df, diff_col).replace(0, np.nan)
        ret_1    = self._clean_inf(np.log(close / close.shift(1).replace(0, np.nan)), 0.0)
        ret_norm = (ret_1 / 0.05).clip(-1.0, 1.0)  # normalised to [-1, 1]

        body_ratio   = self._get_series(df, "Body_Ratio", 0.0)
        close_pos    = self._get_series(df, "Close_Pos", 0.5)
        lower_wick_r = self._get_series(df, "Lower_Wick_R", 0.0)
        high_close_dd= self._get_series(df, "High_Close_DD", 0.0)
        rel_high     = self._get_series(df, "Rel_High", 0.0)
        bar_dir      = self._get_series(df, "Bar_Dir", 0.0)
        ...
        df = self._safe_concat(df, {"Candle_Bull_Score": score.clip(0.0, 1.0).fillna(0.5)})
    ...
    def _calculate_price_velocity(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        close = self._get_series(df, diff_col)
        if close is None:
            return df
        ...
        if isinstance(df.index, pd.DatetimeIndex):
            ...
            time_diff_s = self._get_series(df, "Time_Diff_From_Last_Swing_Low")
            if time_diff_s is not None:
                mean_bar_s = time_diff_s.replace(0, np.nan).median()
        ...
        if atr_series is not None:
            atr_per_s = (atr_series / elapsed_s * window).replace(0, np.nan)
    ...
    def _calculate_volatility_regime(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Volatility Regime — normalised volatility state and directional split.
        """
        df = self._deduplicate_df(df)
        diff_col = self.config.diff_column
        bar_diff = (self._get_series(df, diff_col) - self._get_series(df, "Open")).abs()

        # Extract directional moves
        is_up = self._get_series(df, diff_col) >= self._get_series(df, "Open")
        bull_moves = bar_diff.where(is_up, 0.0)
        bear_moves = bar_diff.where(~is_up, 0.0)

        window = self.config.snr_window
        new_cols = {}

        # ── Volatility_Regime: normalised vol vs 95th percentile ──────────────
        if "VIX_20" in df.columns:
            vix = self._get_series(df, "VIX_20").replace(0, np.nan)
            ...
            atr_pct = self._get_series(df, "ATR_Pct").replace(0, np.nan)
        ...
        new_cols["Volatility_Bull"] = self._clean_inf(bull_std / total_std, 0.5).clip(0.0, 1.0)
        new_cols["Volatility_Bear"] = self._clean_inf(bear_std / total_std, 0.5).clip(0.0, 1.0)
    ...
    def _calculate_regime_speed(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate trend-aligned regime speed — pip progress per unit time in trend direction.
        """
        df = self._deduplicate_df(df)
        new_cols = {}

        if "Price_Velocity_Bull" not in df.columns or "Price_Velocity_Bear" not in df.columns:
            logger.warning("⚠️ [TI] _calculate_regime_speed: Price_Velocity_Bull/Bear missing.")
            return df

        vel_bull = self._get_series(df, "Price_Velocity_Bull", 0.0)
        vel_bear = self._get_series(df, "Price_Velocity_Bear", 0.0)

        if "Trend_Strength" in df.columns:
            trend_gate = self._get_series(df, "Trend_Strength", 0.5).clip(0.0, 1.0)
        else:
            trend_gate = self._get_series(df, "Candle_Bull_Score", 0.5).clip(0.0, 1.0) if "Candle_Bull_Score" in df.columns else pd.Series(0.5, index=df.index)
        ...
        speed_bull = (raw_bull / bull_95).clip(0.0, 1.0).fillna(0.0)
        speed_bear = (raw_bear / bear_95).clip(0.0, 1.0).fillna(0.0)
        new_cols["Regime_Speed_Bull"] = speed_bull
        new_cols["Regime_Speed_Bear"] = speed_bear

        if "Supertrend" in df.columns:
            is_bull_regime = self._get_series(df, "Supertrend").astype(bool)
            new_cols["Regime_Speed_Aligned"] = pd.Series(
                np.where(is_bull_regime, speed_bull, speed_bear),
                index=df.index
            )
        else:
            net_vel = self._get_series(df, "Price_Velocity_Net", 0.0)
            new_cols["Regime_Speed_Aligned"] = pd.Series(
                np.where(net_vel >= 0, speed_bull, speed_bear),
                index=df.index
            )

        denom = (speed_bull + speed_bear).replace(0, np.nan)
        divergence = (speed_bull - speed_bear) / denom
    ...
    def _calculate_mean_reversion_score(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Mean Reversion Score [0, 1] — probability of price reverting to mean.
        """
        df = self._deduplicate_df(df)
        new_cols = {}
        ...
        if 'BB_Upper_Diff' in df.columns and 'BB_Lower_Diff' in df.columns:
            bb_u_diff = self._get_series(df, 'BB_Upper_Diff').abs()
            bb_l_diff = self._get_series(df, 'BB_Lower_Diff').abs()
            min_bb_diff = pd.concat([bb_u_diff, bb_l_diff], axis=1).min(axis=1)
            bb_stretch = (1.0 - (min_bb_diff / 100.0)).clip(0.0, 1.0).fillna(0.0)
        
        # ── 2. RSI Extreme (25%) ──
        rsi_extreme = pd.Series(0.0, index=df.index)
        rsi_col = 'RSI' if 'RSI' in df.columns else ('RSI_14' if 'RSI_14' in df.columns else None)
        if rsi_col:
            rsi = self._get_series(df, rsi_col)
            rsi_dist = (rsi - 50.0).abs()
            rsi_extreme = (rsi_dist / 30.0).clip(0.0, 1.0).fillna(0.0)
        
        # ── 3. Volatility Spike (20%) ──
        vol_spike = pd.Series(0.0, index=df.index)
        if 'Volatility_Expansion' in df.columns:
            vol_spike = self._get_series(df, 'Volatility_Expansion').clip(0.0, 1.0).fillna(0.0)
        ...
        close_s = self._get_series(df, 'Close')
        atr_s   = self._get_series(df, 'ATR')
        if ma_col and 'Close' in df.columns and 'ATR' in df.columns:
            ma_s = self._get_series(df, ma_col)
            atr_safe = atr_s.replace(0, np.nan)
            ma_distance = ((close_s - ma_s).abs() / atr_safe).clip(0.0, 1.0).infer_objects(copy=False).fillna(0.0)
        
        # ── 5. TF Deviation (10%) ──
        tf_deviation = pd.Series(0.0, index=df.index)
        if 'Trend_Strength' in df.columns:
            trend_str = self._get_series(df, 'Trend_Strength')
            tf_deviation = (1.0 - trend_str).clip(0.0, 1.0).fillna(0.0)
    ...
    def _calculate_momentum_delta(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not rsi_col:
            logger.warning("⚠️ [TI] _calculate_momentum_delta: No RSI column found, cannot calculate momentum deltas")
            ...
            rsi_s = self._get_series(df, rsi_col)
            momentum = (rsi_s - 50) / 50
    ...
    def _finalize_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Deduplicate columns (Case-Insensitive) to prevent "same-caps" issues and defragment dataframe
        df = df.loc[:, ~df.columns.str.lower().duplicated(keep='first')].copy()
