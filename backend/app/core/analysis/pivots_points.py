import app.core.analysis.indicators as qtpylib
import pandas as pd
...
def pivots_points(dataframe: pd.DataFrame, timeperiod=30, levels=3) -> pd.DataFrame:
    ...
    # Pivot
    typical_price_series = (dataframe["High"] + dataframe["Low"] + dataframe["Close"]) / 3.0
    data["pivot"] = qtpylib.rolling_mean(
        series=pd.Series(index=dataframe.index, data=typical_price_series), 
        window=timeperiod
    )
