...
class ProprietaryModelRuntime:
    ...
    def _contract(self, model_id: str) -> Dict[str, Path]:
        contract = self._contracts.get(model_id)
        if not contract:
            raise ProprietaryModelContractError(
                f"Selected model '{model_id}' has no registered serving artifact contract."
            )
        return contract

    def required_indices(self, model_id: str) -> List[str]:
        try:
            required_features = self._load_feature_names(self._contract(model_id)["feature_map"])
    ...
    def required_pairs(self, model_id: str) -> List[str]:
        pairs: Set[str] = set()
        for index_name in self.required_indices(model_id):
    ...
    def _run_inference(
        ...
        *,
        ...
        """
        from app.core.ml.feature_serving_pipeline import (
        ...
        if model_id == "axe_genesis_v2":
            ...
            named_heads = model_or_v2._map_heads_to_contract(
                raw_named_heads,
                feature_window=feature_window,
                snr_features=snr_features,
            )
            ...
            named_heads = FeatureServingPipeline.map_raw_predictions(model_or_v2, raw_preds)

        logger.info(
            "[AXE Runtime] Inference complete for %s — %d heads produced: %s",
            model_id,
            len(named_heads),
            sorted(named_heads.keys()),
        )

        named_heads = self._inverse_scaled_output_targets(model_id, contract, named_heads)

        # ── 7. Denormalize heads to real-world values ─────────────────────────
        # Load output_transform_spec from the feature map JSON if available,
        # otherwise build it from known head names.
        feature_map_payload = json.loads(contract["feature_map"].read_text())
        scaling_config_from_disk = (
            scaling_config
            or self._coerce_json_dict(feature_map_payload.get("scaling_config"))
        )
        generated_output_spec = build_output_transform_spec(
            target_names=list(named_heads.keys()),
            scaling_config=scaling_config_from_disk,
        )
        persisted_output_spec = scaling_config_from_disk.get("output_transform_spec") or {}
        output_transform_spec = {
            **generated_output_spec,
            **persisted_output_spec,
        }

        # Use reference_close from pipeline (pre-slice) when available;
        # fall back to feature_window last row (legacy path).
        if reference_close is None:
            reference_close = float(feature_window["Close"].iloc[-1]) if "Close" in feature_window.columns else None
        scaling_config_from_disk = self._with_live_ohlc_sigmoid_anchors(
            scaling_config_from_disk,
            feature_window,
        )

        # Detect pip size from reference close (XAUUSD ≈ 2500 → pip = 0.01)
        if reference_close and reference_close > 1000:
            pip_size = 0.01
        elif reference_close and reference_close > 50:
            pip_size = 0.01
        else:
            pip_size = 0.0001
        ...
        # Save raw predictions and denormalization context snapshot for audit & iterative tuning
        try:
            import time
            snapshot_dir = Path('/media/daniel-joseph/Linux_Data/Dan_backup/Projects/JavaScript/fin-dash-buddy-sos/Backend/artifacts/live_inference_debug')
            snapshot_dir.mkdir(parents=True, exist_ok=True)
            snapshot_path = snapshot_dir / f'last_prediction_snapshot_{int(time.time())}.json'
            ...
            snapshot_data = {
                ...
                'model_id': model_id,
                'reference_close': reference_close,
                'scaling_config': _to_json_safe(scaling_config_from_disk),
                'output_transform_spec': _to_json_safe(output_transform_spec),
                'raw_predictions': _to_json_safe(named_heads),
            }
            snapshot_path.write_text(json.dumps(snapshot_data, indent=2))
            logger.info(f'[AXE Runtime] 💾 Saved raw prediction snapshot to {snapshot_path}')
        except Exception as snap_err:
            logger.warning(f'[AXE Runtime] ⚠️ Failed to save prediction snapshot: {snap_err}')

        ui_heads = denorm.denormalize_all(named_heads)

        logger.info(
            "[AXE Runtime] Denormalized heads for %s (ref_close=%.4f): bull_prob=%s mfe=%s mae=%s",
            model_id,
            reference_close or 0.0,
            ui_heads.get("bull_prob"),
            ui_heads.get("mfe"),
            ui_heads.get("mae"),
        )
    ...
    def _with_live_ohlc_sigmoid_anchors(
        ...
        """
        Add live rolling-mean anchors for inverse OHLC sequence denormalization.

        OHLC sequence targets are virtual arrays generated from already-normalized
        dataframe OHLC columns. Training forward normalization is:
            raw_norm = (price - structural_low) / structural_width
            y = sigmoid(k * (raw_norm - rolling_mean(raw_norm, 20)) / abs(rolling_mean))

        The scaler does not contain those virtual target arrays, so output inverse
        needs the live rolling-mean anchor reconstructed from the raw OHLC history.
        """
        if not isinstance(scaling_config, dict):
            return scaling_config
        if scaling_config.get("normalization_method") != "rolling_mean_sigmoid":
            return scaling_config

        sr = scaling_config.get("structural_range") or {}
        try:
            low = float(sr.get("low") or 0.0)
            width = float(sr.get("width") or 0.0)
        except (TypeError, ValueError):
            return scaling_config
        if width <= 0 or feature_window is None or not hasattr(feature_window, "columns"):
            return scaling_config

        window = int((sr.get("window") or 20))
        window = 20 if window <= 0 else min(20, window)
        head_to_col = {
            "open_sequence": "Open",
            "high_sequence": "High",
            "low_sequence": "Low",
            "close_sequence": "Close",
            "support_trendline": "Close",
            "resist_trendline": "Close",
        }
        anchors: Dict[str, float] = {}
        for head, col in head_to_col.items():
            if col not in feature_window.columns:
                continue
            series = pd.to_numeric(feature_window[col], errors="coerce").dropna()
            if series.empty:
                continue
            raw_norm = ((series.tail(window) - low) / width).replace([np.inf, -np.inf], np.nan).dropna()
            if raw_norm.empty:
                continue
            anchor = float(raw_norm.mean())
            if np.isfinite(anchor):
                anchors[head] = max(1e-6, anchor)

        if not anchors:
            return scaling_config

        out = dict(scaling_config)
        out["ohlc_rolling_mean_anchors"] = anchors
        logger.info("[AXE Runtime] Live OHLC sigmoid inverse anchors: %s", anchors)
        return out
    ...
    def _inverse_scaled_output_targets(
        ...
        """
        Invert target-column scaling for scalar regression heads backed by adv_target_*.

        Training includes MFE/MAE return-fraction targets in SelectiveScaler.
        Those heads must be restored to raw return-fraction space before
        OutputDenormalizer converts them to price points. Classification,
        probability, and zone heads are already emitted in model/UI domain by
        the V2 runtime and must not be inverse-transformed through the target
        scaler.
        """
        try:
            from app.core.ml.output_spec import V8_3_NPZ_TARGET_KEY_MAP

            target_scaled_regression_heads = {"mfe", "mae"}
            target_for_head = {
                model_head: raw_key
                for raw_key, model_head in V8_3_NPZ_TARGET_KEY_MAP.items()
                if raw_key.startswith("adv_target_")
            }
            scaler = self._load_scaler_cached(model_id, contract["scaler"])
            scaler_cols: List[str] = list(getattr(scaler, "feature_names", None) or getattr(scaler, "all_cols", []) or [])
            if not scaler_cols:
                return named_heads

            updates: Dict[str, Any] = {}
            for head, value in named_heads.items():
                if head not in target_scaled_regression_heads:
                    continue
                target_col = target_for_head.get(head)
                if not target_col or target_col not in scaler_cols:
                    continue
                arr = np.asarray(value)
                if arr.size != 1:
                    continue
                try:
                    raw_scaled = float(arr.reshape(-1)[0])
                except (TypeError, ValueError):
                    continue

                scale_row = pd.DataFrame(
                    {col: [0.0] for col in scaler_cols},
                    columns=scaler_cols,
                )
                scale_row.at[0, target_col] = raw_scaled
                inv = scaler.inverse_transform(scale_row)
                inv_df = inv if isinstance(inv, pd.DataFrame) else pd.DataFrame(np.asarray(inv), columns=scaler_cols)
                updates[head] = float(inv_df[target_col].iloc[0])

            if updates:
                out = dict(named_heads)
                out.update(updates)
                logger.info(
                    "[AXE Runtime] Inverse-transformed %d scaled regression output target head(s): %s",
                    len(updates),
                    sorted(updates.keys()),
                )
                return out
            ...
            logger.warning(
                "[AXE Runtime] Failed to inverse-transform scaled output targets for %s: %s(%r)",
                model_id,
                type(err).__name__,
                err,
            )
