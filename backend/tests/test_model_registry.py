...
def test_train_split_scaler_only_on_train():
    ...
    result = train_from_synthetic(
        SyntheticTrainConfig(num_candles=1500, warmup_bars=80, train_steps=2, batch_size=8, persist=False, seed=3),
        db=None,
    )
...
def test_val_eval_metrics_present_in_result():
    result = train_from_synthetic(
        SyntheticTrainConfig(num_candles=1500, warmup_bars=80, train_steps=2, batch_size=8, persist=False, seed=5),
        db=None,
    )
...
def test_registry_pretrain_creates_active_checkpoint(db_session):
    ...
    result = registry.pretrain(
        ...
        scope="test-tsla-v1",
        db=db_session,
        config_overrides={"num_candles": 1500, "warmup_bars": 80, "train_steps": 2, "batch_size": 8, "seed": 10},
    )
...
def test_registry_list_models_filters_by_symbol(db_session):
    ...
    registry.pretrain("AAPL", "scope-a", db_session,
                      config_overrides={"num_candles": 1500, "warmup_bars": 80, "train_steps": 2, "batch_size": 8})
...
def test_registry_evaluate_returns_eval_metrics(db_session):
    ...
    result = registry.pretrain("QQQ", "scope-qqq", db_session,
                               config_overrides={"num_candles": 1500, "warmup_bars": 80, "train_steps": 2, "batch_size": 8, "seed": 5})
...
def test_registry_set_active_hot_swaps_model(db_session):
    ...
    registry = MetaLearnerModelRegistry()
    cfg = {"num_candles": 1500, "warmup_bars": 80, "train_steps": 2, "batch_size": 8}
